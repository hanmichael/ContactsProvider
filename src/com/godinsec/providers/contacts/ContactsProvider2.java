...
import com.godinsec.providers.contacts.reflect.ProjectionMap;
import com.godinsec.providers.contacts.reflect.SyncStateContentProviderHelper;
import com.godinsec.providers.contacts.reflect.MoreCloseables;
import com.android.internal.util.ArrayUtils;
import com.godinsec.providers.contacts.ContactLookupKey.LookupKeySegment;
import com.godinsec.providers.contacts.ContactsDatabaseHelper.AccountsColumns;
import com.godinsec.providers.contacts.ContactsDatabaseHelper.AggregatedPresenceColumns;
import com.godinsec.providers.contacts.ContactsDatabaseHelper.AggregationExceptionColumns;
import com.godinsec.providers.contacts.ContactsDatabaseHelper.Clauses;
import com.godinsec.providers.contacts.ContactsDatabaseHelper.ContactsColumns;
import com.godinsec.providers.contacts.ContactsDatabaseHelper.ContactsStatusUpdatesColumns;
import com.godinsec.providers.contacts.ContactsDatabaseHelper.DataColumns;
import com.godinsec.providers.contacts.ContactsDatabaseHelper.DataUsageStatColumns;
import com.godinsec.providers.contacts.ContactsDatabaseHelper.DbProperties;
import com.godinsec.providers.contacts.ContactsDatabaseHelper.GroupsColumns;
import com.godinsec.providers.contacts.ContactsDatabaseHelper.Joins;
import com.godinsec.providers.contacts.ContactsDatabaseHelper.NameLookupColumns;
import com.godinsec.providers.contacts.ContactsDatabaseHelper.NameLookupType;
import com.godinsec.providers.contacts.ContactsDatabaseHelper.PhoneLookupColumns;
import com.godinsec.providers.contacts.ContactsDatabaseHelper.PhotoFilesColumns;
import com.godinsec.providers.contacts.ContactsDatabaseHelper.PresenceColumns;
import com.godinsec.providers.contacts.ContactsDatabaseHelper.Projections;
import com.godinsec.providers.contacts.ContactsDatabaseHelper.RawContactsColumns;
import com.godinsec.providers.contacts.ContactsDatabaseHelper.SearchIndexColumns;
import com.godinsec.providers.contacts.ContactsDatabaseHelper.SettingsColumns;
import com.godinsec.providers.contacts.ContactsDatabaseHelper.StatusUpdatesColumns;
import com.godinsec.providers.contacts.ContactsDatabaseHelper.StreamItemPhotosColumns;
import com.godinsec.providers.contacts.ContactsDatabaseHelper.StreamItemsColumns;
import com.godinsec.providers.contacts.ContactsDatabaseHelper.Tables;
import com.godinsec.providers.contacts.ContactsDatabaseHelper.ViewGroupsColumns;
import com.godinsec.providers.contacts.ContactsDatabaseHelper.Views;
import com.godinsec.providers.contacts.SearchIndexManager.FtsQueryBuilder;
import com.godinsec.providers.contacts.aggregation.ContactAggregator;
import com.godinsec.providers.contacts.aggregation.ProfileAggregator;
import com.godinsec.providers.contacts.aggregation.ContactAggregator.AggregationSuggestionParameter;
import com.godinsec.providers.contacts.aggregation.util.CommonNicknameCache;
import com.godinsec.providers.contacts.database.ContactsTableUtil;
import com.godinsec.providers.contacts.database.DeletedContactsTableUtil;
import com.godinsec.providers.contacts.database.MoreDatabaseUtils;
import com.godinsec.providers.contacts.util.Clock;
import com.godinsec.providers.contacts.util.DbQueryUtils;
import com.godinsec.providers.contacts.util.NeededForTesting;
import com.godinsec.providers.contacts.util.UserUtils;
...
public class ContactsProvider2 extends AbstractContactsProvider
    ...
    /*for godinsec provider begin*/

    public static final String GODINSEC_AUTHORITY = "com.godinsec.contacts";
    public static final Uri GODINSEC_AUTHORITY_URI = Uri.parse("content://" + GODINSEC_AUTHORITY);

    public static final Uri PROFILE_CONTENT_URI = Uri.withAppendedPath(GODINSEC_AUTHORITY_URI, "profile");
    public static final Uri DISPLAYPHOTO_CONTENT_URI = Uri.withAppendedPath(GODINSEC_AUTHORITY_URI, "display_photo");

    public static final Uri DATA_CONTENT_URI = Uri.withAppendedPath(GODINSEC_AUTHORITY_URI, "data");
    public static final Uri CP_STATUS_CONTENT_URI = Uri.withAppendedPath(GODINSEC_AUTHORITY_URI, "provider_status");

    public static final Uri SETTINGS_CONTENT_URI = Uri.withAppendedPath(GODINSEC_AUTHORITY_URI, "settings");

    public static final Uri STREAM_ITEMS_CONTENT_URI = Uri.withAppendedPath(GODINSEC_AUTHORITY_URI, "stream_items");

    public static final Uri CORP_CONTENT_URI = Uri.withAppendedPath(GODINSEC_AUTHORITY_URI, "contacts_corp");
    public static final Uri CONTACTS_CONTENT_URI = Uri.withAppendedPath(GODINSEC_AUTHORITY_URI, "contacts");
    public static final Uri RAW_CONTENT_URI = Uri.withAppendedPath(GODINSEC_AUTHORITY_URI, "raw_contacts");
    public static final Uri RAW_ENITIES_PROFILE_CONTENT_URI = Uri.withAppendedPath(PROFILE_CONTENT_URI, "raw_contact_entities");
    
    /*for godinsec provider end*/
    ...
    private static final ProjectionMap sStreamItemPhotosProjectionMap = ProjectionMap.builder()
            ...
            .add(StreamItemPhotos.PHOTO_URI,
                    "'" + DISPLAYPHOTO_CONTENT_URI + "'||'/'||" + StreamItemPhotos.PHOTO_FILE_ID)
    ...
    static {
        ...
        // readability.
        matcher.addURI(GODINSEC_AUTHORITY, "contacts", CONTACTS);
        matcher.addURI(GODINSEC_AUTHORITY, "contacts/#", CONTACTS_ID);
        matcher.addURI(GODINSEC_AUTHORITY, "contacts/#/data", CONTACTS_ID_DATA);
        matcher.addURI(GODINSEC_AUTHORITY, "contacts/#/entities", CONTACTS_ID_ENTITIES);
        matcher.addURI(GODINSEC_AUTHORITY, "contacts/#/suggestions",
                AGGREGATION_SUGGESTIONS);
        matcher.addURI(GODINSEC_AUTHORITY, "contacts/#/suggestions/*",
                AGGREGATION_SUGGESTIONS);
        matcher.addURI(GODINSEC_AUTHORITY, "contacts/#/photo", CONTACTS_ID_PHOTO);
        matcher.addURI(GODINSEC_AUTHORITY, "contacts/#/display_photo",
                CONTACTS_ID_DISPLAY_PHOTO);

        // Special URIs that refer to contact pictures in the corp CP2.
        matcher.addURI(GODINSEC_AUTHORITY, "contacts_corp/#/photo", CONTACTS_ID_PHOTO_CORP);
        matcher.addURI(GODINSEC_AUTHORITY, "contacts_corp/#/display_photo",
                CONTACTS_ID_DISPLAY_PHOTO_CORP);

        matcher.addURI(GODINSEC_AUTHORITY, "contacts/#/stream_items",
                CONTACTS_ID_STREAM_ITEMS);
        matcher.addURI(GODINSEC_AUTHORITY, "contacts/filter", CONTACTS_FILTER);
        matcher.addURI(GODINSEC_AUTHORITY, "contacts/filter/*", CONTACTS_FILTER);
        matcher.addURI(GODINSEC_AUTHORITY, "contacts/lookup/*", CONTACTS_LOOKUP);
        matcher.addURI(GODINSEC_AUTHORITY, "contacts/lookup/*/data", CONTACTS_LOOKUP_DATA);
        matcher.addURI(GODINSEC_AUTHORITY, "contacts/lookup/*/photo",
                CONTACTS_LOOKUP_PHOTO);
        matcher.addURI(GODINSEC_AUTHORITY, "contacts/lookup/*/#", CONTACTS_LOOKUP_ID);
        matcher.addURI(GODINSEC_AUTHORITY, "contacts/lookup/*/#/data",
                CONTACTS_LOOKUP_ID_DATA);
        matcher.addURI(GODINSEC_AUTHORITY, "contacts/lookup/*/#/photo",
                CONTACTS_LOOKUP_ID_PHOTO);
        matcher.addURI(GODINSEC_AUTHORITY, "contacts/lookup/*/display_photo",
                CONTACTS_LOOKUP_DISPLAY_PHOTO);
        matcher.addURI(GODINSEC_AUTHORITY, "contacts/lookup/*/#/display_photo",
                CONTACTS_LOOKUP_ID_DISPLAY_PHOTO);
        matcher.addURI(GODINSEC_AUTHORITY, "contacts/lookup/*/entities",
                CONTACTS_LOOKUP_ENTITIES);
        matcher.addURI(GODINSEC_AUTHORITY, "contacts/lookup/*/#/entities",
                CONTACTS_LOOKUP_ID_ENTITIES);
        matcher.addURI(GODINSEC_AUTHORITY, "contacts/lookup/*/stream_items",
                CONTACTS_LOOKUP_STREAM_ITEMS);
        matcher.addURI(GODINSEC_AUTHORITY, "contacts/lookup/*/#/stream_items",
                CONTACTS_LOOKUP_ID_STREAM_ITEMS);
        matcher.addURI(GODINSEC_AUTHORITY, "contacts/as_vcard/*", CONTACTS_AS_VCARD);
        matcher.addURI(GODINSEC_AUTHORITY, "contacts/as_multi_vcard/*",
                CONTACTS_AS_MULTI_VCARD);
        matcher.addURI(GODINSEC_AUTHORITY, "contacts/strequent/", CONTACTS_STREQUENT);
        matcher.addURI(GODINSEC_AUTHORITY, "contacts/strequent/filter/*",
                CONTACTS_STREQUENT_FILTER);
        matcher.addURI(GODINSEC_AUTHORITY, "contacts/group/*", CONTACTS_GROUP);
        matcher.addURI(GODINSEC_AUTHORITY, "contacts/frequent", CONTACTS_FREQUENT);
        matcher.addURI(GODINSEC_AUTHORITY, "contacts/delete_usage", CONTACTS_DELETE_USAGE);

        matcher.addURI(GODINSEC_AUTHORITY, "raw_contacts", RAW_CONTACTS);
        matcher.addURI(GODINSEC_AUTHORITY, "raw_contacts/#", RAW_CONTACTS_ID);
        matcher.addURI(GODINSEC_AUTHORITY, "raw_contacts/#/data", RAW_CONTACTS_ID_DATA);
        matcher.addURI(GODINSEC_AUTHORITY, "raw_contacts/#/display_photo",
                RAW_CONTACTS_ID_DISPLAY_PHOTO);
        matcher.addURI(GODINSEC_AUTHORITY, "raw_contacts/#/entity", RAW_CONTACT_ID_ENTITY);
        matcher.addURI(GODINSEC_AUTHORITY, "raw_contacts/#/stream_items",
                RAW_CONTACTS_ID_STREAM_ITEMS);
        matcher.addURI(GODINSEC_AUTHORITY, "raw_contacts/#/stream_items/#",
                RAW_CONTACTS_ID_STREAM_ITEMS_ID);

        matcher.addURI(GODINSEC_AUTHORITY, "raw_contact_entities", RAW_CONTACT_ENTITIES);

        matcher.addURI(GODINSEC_AUTHORITY, "data", DATA);
        matcher.addURI(GODINSEC_AUTHORITY, "data/#", DATA_ID);
        matcher.addURI(GODINSEC_AUTHORITY, "data/phones", PHONES);
        matcher.addURI(GODINSEC_AUTHORITY, "data/phones/#", PHONES_ID);
        matcher.addURI(GODINSEC_AUTHORITY, "data/phones/filter", PHONES_FILTER);
        matcher.addURI(GODINSEC_AUTHORITY, "data/phones/filter/*", PHONES_FILTER);
        matcher.addURI(GODINSEC_AUTHORITY, "data/emails", EMAILS);
        matcher.addURI(GODINSEC_AUTHORITY, "data/emails/#", EMAILS_ID);
        matcher.addURI(GODINSEC_AUTHORITY, "data/emails/lookup", EMAILS_LOOKUP);
        matcher.addURI(GODINSEC_AUTHORITY, "data/emails/lookup/*", EMAILS_LOOKUP);
        matcher.addURI(GODINSEC_AUTHORITY, "data/emails/filter", EMAILS_FILTER);
        matcher.addURI(GODINSEC_AUTHORITY, "data/emails/filter/*", EMAILS_FILTER);
        matcher.addURI(GODINSEC_AUTHORITY, "data/postals", POSTALS);
        matcher.addURI(GODINSEC_AUTHORITY, "data/postals/#", POSTALS_ID);
        /** "*" is in CSV form with data IDs ("123,456,789") */
        matcher.addURI(GODINSEC_AUTHORITY, "data/usagefeedback/*", DATA_USAGE_FEEDBACK_ID);
        matcher.addURI(GODINSEC_AUTHORITY, "data/callables/", CALLABLES);
        matcher.addURI(GODINSEC_AUTHORITY, "data/callables/#", CALLABLES_ID);
        matcher.addURI(GODINSEC_AUTHORITY, "data/callables/filter", CALLABLES_FILTER);
        matcher.addURI(GODINSEC_AUTHORITY, "data/callables/filter/*", CALLABLES_FILTER);

        matcher.addURI(GODINSEC_AUTHORITY, "data/contactables/", CONTACTABLES);
        matcher.addURI(GODINSEC_AUTHORITY, "data/contactables/filter", CONTACTABLES_FILTER);
        matcher.addURI(GODINSEC_AUTHORITY, "data/contactables/filter/*",
                CONTACTABLES_FILTER);

        matcher.addURI(GODINSEC_AUTHORITY, "groups", GROUPS);
        matcher.addURI(GODINSEC_AUTHORITY, "groups/#", GROUPS_ID);
        matcher.addURI(GODINSEC_AUTHORITY, "groups_summary", GROUPS_SUMMARY);

        matcher.addURI(GODINSEC_AUTHORITY, SyncStateContentProviderHelper.PATH, SYNCSTATE);
        matcher.addURI(GODINSEC_AUTHORITY, SyncStateContentProviderHelper.PATH + "/#",
                SYNCSTATE_ID);
        matcher.addURI(GODINSEC_AUTHORITY, "profile/" + SyncStateContentProviderHelper.PATH,
                PROFILE_SYNCSTATE);
        matcher.addURI(GODINSEC_AUTHORITY,
                "profile/" + SyncStateContentProviderHelper.PATH + "/#",
                PROFILE_SYNCSTATE_ID);

        matcher.addURI(GODINSEC_AUTHORITY, "phone_lookup/*", PHONE_LOOKUP);
        matcher.addURI(GODINSEC_AUTHORITY, "phone_lookup_enterprise/*",
                PHONE_LOOKUP_ENTERPRISE);
        matcher.addURI(GODINSEC_AUTHORITY, "aggregation_exceptions",
                AGGREGATION_EXCEPTIONS);
        matcher.addURI(GODINSEC_AUTHORITY, "aggregation_exceptions/*",
                AGGREGATION_EXCEPTION_ID);

        matcher.addURI(GODINSEC_AUTHORITY, "settings", SETTINGS);

        matcher.addURI(GODINSEC_AUTHORITY, "status_updates", STATUS_UPDATES);
        matcher.addURI(GODINSEC_AUTHORITY, "status_updates/#", STATUS_UPDATES_ID);

        matcher.addURI(GODINSEC_AUTHORITY, SearchManager.SUGGEST_URI_PATH_QUERY,
                SEARCH_SUGGESTIONS);
        matcher.addURI(GODINSEC_AUTHORITY, SearchManager.SUGGEST_URI_PATH_QUERY + "/*",
                SEARCH_SUGGESTIONS);
        matcher.addURI(GODINSEC_AUTHORITY, SearchManager.SUGGEST_URI_PATH_SHORTCUT + "/*",
                SEARCH_SHORTCUT);

        matcher.addURI(GODINSEC_AUTHORITY, "provider_status", PROVIDER_STATUS);

        matcher.addURI(GODINSEC_AUTHORITY, "directories", DIRECTORIES);
        matcher.addURI(GODINSEC_AUTHORITY, "directories/#", DIRECTORIES_ID);

        matcher.addURI(GODINSEC_AUTHORITY, "complete_name", COMPLETE_NAME);

        matcher.addURI(GODINSEC_AUTHORITY, "profile", PROFILE);
        matcher.addURI(GODINSEC_AUTHORITY, "profile/entities", PROFILE_ENTITIES);
        matcher.addURI(GODINSEC_AUTHORITY, "profile/data", PROFILE_DATA);
        matcher.addURI(GODINSEC_AUTHORITY, "profile/data/#", PROFILE_DATA_ID);
        matcher.addURI(GODINSEC_AUTHORITY, "profile/photo", PROFILE_PHOTO);
        matcher.addURI(GODINSEC_AUTHORITY, "profile/display_photo", PROFILE_DISPLAY_PHOTO);
        matcher.addURI(GODINSEC_AUTHORITY, "profile/as_vcard", PROFILE_AS_VCARD);
        matcher.addURI(GODINSEC_AUTHORITY, "profile/raw_contacts", PROFILE_RAW_CONTACTS);
        matcher.addURI(GODINSEC_AUTHORITY, "profile/raw_contacts/#",
                PROFILE_RAW_CONTACTS_ID);
        matcher.addURI(GODINSEC_AUTHORITY, "profile/raw_contacts/#/data",
                PROFILE_RAW_CONTACTS_ID_DATA);
        matcher.addURI(GODINSEC_AUTHORITY, "profile/raw_contacts/#/entity",
                PROFILE_RAW_CONTACTS_ID_ENTITIES);
        matcher.addURI(GODINSEC_AUTHORITY, "profile/status_updates",
                PROFILE_STATUS_UPDATES);
        matcher.addURI(GODINSEC_AUTHORITY, "profile/raw_contact_entities",
                PROFILE_RAW_CONTACT_ENTITIES);

        matcher.addURI(GODINSEC_AUTHORITY, "stream_items", STREAM_ITEMS);
        matcher.addURI(GODINSEC_AUTHORITY, "stream_items/photo", STREAM_ITEMS_PHOTOS);
        matcher.addURI(GODINSEC_AUTHORITY, "stream_items/#", STREAM_ITEMS_ID);
        matcher.addURI(GODINSEC_AUTHORITY, "stream_items/#/photo", STREAM_ITEMS_ID_PHOTOS);
        matcher.addURI(GODINSEC_AUTHORITY, "stream_items/#/photo/#",
                STREAM_ITEMS_ID_PHOTOS_ID);
        matcher.addURI(GODINSEC_AUTHORITY, "stream_items_limit", STREAM_ITEMS_LIMIT);

        matcher.addURI(GODINSEC_AUTHORITY, "display_photo/#", DISPLAY_PHOTO_ID);
        matcher.addURI(GODINSEC_AUTHORITY, "photo_dimensions", PHOTO_DIMENSIONS);

        matcher.addURI(GODINSEC_AUTHORITY, "deleted_contacts", DELETED_CONTACTS);
        matcher.addURI(GODINSEC_AUTHORITY, "deleted_contacts/#", DELETED_CONTACTS_ID);
    }
    ...
    private boolean initialize() {
        ...
        profileInfo.authority = GODINSEC_AUTHORITY;
            ...
            try {
                ...
                for (long missingPhotoId : missingPhotoIds) {
                    if (photoFileIdToDataId.containsKey(missingPhotoId)) {
                        ...
                        updateData(ContentUris.withAppendedId(DATA_CONTENT_URI, dataId),
    ...
    protected void notifyChange(boolean syncToNetwork) {
        getContext().getContentResolver().notifyChange(GODINSEC_AUTHORITY_URI, null,
                syncToNetwork);
    }
    ...
    protected void setProviderStatus(int status) {
        if (mProviderStatus != status) {
            ...
            getContext().getContentResolver().notifyChange(CP_STATUS_CONTENT_URI, null, false);
        }
    }
    ...
    private int deleteData(String selection, String[] selectionArgs, boolean callerIsSyncAdapter) {
        ...
        Uri dataUri = inProfileMode()
                ? Uri.withAppendedPath(PROFILE_CONTENT_URI, RawContacts.Data.CONTENT_DIRECTORY)
                : DATA_CONTENT_URI;
    ...
    public int deleteData(long dataId, String[] allowedMimeTypes) {
        ...
        Cursor c = query(DATA_CONTENT_URI, DataRowHandler.DataDeleteQuery.COLUMNS, Data._ID + "=?",
    ...
    private long insertSettings(ContentValues values) {
        ...
        Uri.Builder settingsUri = SETTINGS_CONTENT_URI.buildUpon();
        if (accountName != null) {
        ...
        if (inputValues.containsKey(StatusUpdates.STATUS)) {
            ...
            if (TextUtils.isEmpty(status)) {
                ...
                if (rawContactId != -1 && !TextUtils.isEmpty(status)) {
                    ...
                    Uri streamUri = STREAM_ITEMS_CONTENT_URI;
    ...
    private int updateGroups(ContentValues originalValues, String selectionWithId,
        ...
        if (flagIsSet(updatedValues, Groups.SHOULD_SYNC)) {
            for (Account account : affectedAccounts) {
                ContentResolver.requestSync(account, GODINSEC_AUTHORITY, new Bundle());
            }
        }
        return returnCount;
    }
    ...
    protected boolean isContactsAccount(Account account) {
        ...
        try {
            return cs.getIsSyncable(account, GODINSEC_AUTHORITY) > 0;
            ...
            case CONTACTS_STREQUENT: {
                ...
                if (cursor != null) {
                    cursor.setNotificationUri(
                            getContext().getContentResolver(), GODINSEC_AUTHORITY_URI);
                }
                return cursor;
            }
    ...
    private Cursor query(final SQLiteDatabase db, SQLiteQueryBuilder qb, String[] projection,
        ...
        if (c != null) {
            c.setNotificationUri(getContext().getContentResolver(), GODINSEC_AUTHORITY_URI);
        }
        return c;
    }
    ...
    private AssetFileDescriptor openAssetFileInner(
        ...
        switch (match) {
            ...
            case CONTACTS_AS_MULTI_VCARD: {
                ...
                Uri queryUri = CONTACTS_CONTENT_URI;
    ...
    private AssetFileDescriptor openCorpContactPicture(long contactId, Uri uri, String mode,
        ...
        final Uri corpUri = maybeAddUserId(
                ContentUris.appendId(CONTACTS_CONTENT_URI.buildUpon(), contactId)
            ...
            try {
                ...
                if (b != null) {
                    ...
                    if (mDataId != 0) {
                        ...
                        update(ContentUris.withAppendedId(DATA_CONTENT_URI, mDataId),
                        ...
                        insert(RAW_CONTENT_URI.buildUpon()
    ...
    public boolean isWritableAccountWithDataSet(String accountTypeAndDataSet) {
        ...
        try {
            ...
            for (SyncAdapterType sync : contentService.getSyncAdapterTypes()) {
                if (GODINSEC_AUTHORITY.equals(sync.authority) &&
    ...
    /* package */
    static boolean readBooleanQueryParameter(
