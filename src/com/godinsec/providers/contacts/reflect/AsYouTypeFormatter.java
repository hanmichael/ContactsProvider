...
import com.godinsec.providers.contacts.reflect.Phonemetadata.NumberFormat;
import com.godinsec.providers.contacts.reflect.Phonemetadata.PhoneMetadata;
 ...
 * {@link PhoneNumberUtil#getAsYouTypeFormatter}. After that, digits can be
 * added by invoking {@link #inputDigit} on the formatter instance, and the
 * partially formatted phone number will be returned each time a digit is added.
 * {@link #clear} can be invoked before formatting a new number.
 * 
 * <p>
...
public class AsYouTypeFormatter {
	...
	// Set to true when users enter their own formatting. AsYouTypeFormatter
	...
	// significant number, since
	// we have either detected a national prefix or an international dialing
	// prefix. When this is
	...
	// Character used when appropriate to separate a prefix, such as a long NDD
	// or a country calling
	// code, from the national number.
	private static final char SEPARATOR_BEFORE_NATIONAL_NUMBER = ' ';
	private static final PhoneMetadata EMPTY_METADATA = new PhoneMetadata()
	...
	private static final Pattern CHARACTER_CLASS_PATTERN = Pattern
			.compile("\\[([^\\[\\]])*\\]");
	// Any digit in a regular expression that actually denotes a digit. For
	// example, in the regular
	// expression 80[0-2]\d{6,10}, the first 2 digits (8 and 0) are standalone
	// digits, but the rest
	// are not.
	// Two look-aheads are needed because the number following \\d could be a
	// two-digit number, since
	// the phone number can be as long as 15 digits.
	private static final Pattern STANDALONE_DIGIT_PATTERN = Pattern
			.compile("\\d(?=[^,}][^,}])");

	// A pattern that is used to determine if a numberFormat under
	// availableFormats is eligible to be
	// used by the AYTF. It is eligible when the format element under
	// numberFormat contains groups of
	// the dollar sign followed by a single digit, separated by valid phone
	// number punctuation. This
	// prevents invalid punctuation (such as the star sign in Israeli star
	// numbers) getting into the
	// output of the AYTF.
	private static final Pattern ELIGIBLE_FORMAT_PATTERN = Pattern.compile("["
			+ PhoneNumberUtil.VALID_PUNCTUATION + "]*" + "(\\$\\d" + "["
			+ PhoneNumberUtil.VALID_PUNCTUATION + "]*)+");
	// A set of characters that, if found in a national prefix formatting rules,
	// are an indicator to
	// us that we should separate the national prefix from the number when
	// formatting.
	private static final Pattern NATIONAL_PREFIX_SEPARATORS_PATTERN = Pattern
			.compile("[- ]");

	// This is the minimum length of national number accrued that is required to
	// trigger the
	// formatter. The first element of the leadingDigitsPattern of each
	// numberFormat contains a
	...
	private static final Pattern DIGIT_PATTERN = Pattern
			.compile(DIGIT_PLACEHOLDER);
	...
	// and it is formatted (e.g. with space inserted). For example, this can
	// contain IDD, country
	// code, and/or NDD, etc.
	private StringBuilder prefixBeforeNationalNumber = new StringBuilder();
	private boolean shouldAddSpaceAfterNationalPrefix = false;
	// This contains the national prefix that has been extracted. It contains
	// only digits without
	// formatting.
	 ...
	 * {@link PhoneNumberUtil#getAsYouTypeFormatter}.
	 * 
	 * @param regionCode
	...
	// same country calling
	// code. Therefore, we return the metadata for "main" region for this
	...
	private boolean maybeCreateNewTemplate() {
		...
		while (it.hasNext()) {
			...
			if (createFormattingTemplate(numberFormat)) {
				...
				shouldAddSpaceAfterNationalPrefix = NATIONAL_PREFIX_SEPARATORS_PATTERN
						.matcher(numberFormat.getNationalPrefixFormattingRule())
						.find();
				// With a new formatting template, the matched position using
				// the old template needs to be
				// reset.
				lastMatchPosition = 0;
				return true;
	...
	private void getAvailableFormats(String leadingThreeDigits) {
		List<NumberFormat> formatList = (isCompleteNumber && currentMetaData
				.intlNumberFormatSize() > 0) ? currentMetaData
				.intlNumberFormats() : currentMetaData.numberFormats();
		boolean nationalPrefixIsUsedByCountry = currentMetaData
				.hasNationalPrefix();
		for (NumberFormat format : formatList) {
			if (!nationalPrefixIsUsedByCountry
					|| isCompleteNumber
					|| format.isNationalPrefixOptionalWhenFormatting()
					|| phoneUtil.formattingRuleHasFirstGroupOnly(format
	...
	private void narrowDownPossibleFormats(String leadingDigits) {
		int indexOfLeadingDigitsPattern = leadingDigits.length()
				- MIN_LEADING_DIGITS_LENGTH;
		Iterator<NumberFormat> it = possibleFormats.iterator();
		while (it.hasNext()) {
			NumberFormat format = it.next();
			if (format.leadingDigitsPatternSize() > indexOfLeadingDigitsPattern) {
				Pattern leadingDigitsPattern = regexCache
						.getPatternForRegex(format
								.getLeadingDigitsPattern(indexOfLeadingDigitsPattern));
				Matcher m = leadingDigitsPattern.matcher(leadingDigits);
				if (!m.lookingAt()) {
					it.remove();
				}
			} // else the particular format has no more specific
				// leadingDigitsPattern, and it should be
				// retained.
		}
	}
	...
	private boolean createFormattingTemplate(NumberFormat format) {
		...
		numberPattern = CHARACTER_CLASS_PATTERN.matcher(numberPattern)
				.replaceAll("\\\\d");

		// Replace any standalone digit (not the one in d{}) with \d
		numberPattern = STANDALONE_DIGIT_PATTERN.matcher(numberPattern)
				.replaceAll("\\\\d");
		formattingTemplate.setLength(0);
		String tempTemplate = getFormattingTemplate(numberPattern,
				format.getFormat());
	...
	// partial number where
	// digits are added one by one.
	private String getFormattingTemplate(String numberPattern,
			String numberFormat) {
		// Creates a phone number consisting only of the digit 9 that matches
		// the
		// numberPattern by applying the pattern to the longestPhoneNumber
		// string.
		String longestPhoneNumber = "999999999999999";
		Matcher m = regexCache.getPatternForRegex(numberPattern).matcher(
				longestPhoneNumber);
		m.find(); // this will always succeed
		String aPhoneNumber = m.group();
		// No formatting template can be created if the number of digits entered
		// so far is longer than
	 ...
	 * 
	 * @param nextChar
	 *            the most recently entered digit of a phone number. Formatting
	 *            characters are allowed, but as soon as they are encountered
	 *            this method formats the number as entered and not
	 *            "as you type" anymore. Full width digits and Arabic-indic
	 ...
	 * {@link #getRememberedPosition}. The remembered position will be
	 * automatically adjusted if additional formatting characters are later
	...
	private String inputDigitWithOptionToRememberPosition(char nextChar,
		...
		if (!ableToFormat) {
			...
			// formatting chars have been
			// entered, it can be due to really long IDDs or NDDs. If that is
			// the case, we might be able
			...
			if (inputHasFormatting) {
				...
				// readability. We don't set shouldAddSpaceAfterNationalPrefix
				// to true, since we don't want
		...
		// MIN_LEADING_DIGITS_LENGTH digits (the plus
		// sign is counted as a digit as well for this purpose) have been
		// entered.
		...
		case 3:
			...
			} else { // No IDD or plus sign is found, might be entering in
						// national format.
		...
		default:
			...
			if (possibleFormats.size() > 0) { // The formatting pattern is
												// already chosen.
				String tempNationalNumber = inputDigitHelper(nextChar);
				// See if the accrued digits can be formatted properly already.
				// If not, use the results
				// from inputDigitHelper, which does formatting based on the
				...
				return ableToFormat ? appendNationalNumber(tempNationalNumber)
	...
	// shorter NDD doesn't result
	// in a number we can format, we try to see if we can extract a longer
	// version here.
	private boolean ableToExtractLongerNdd() {
		if (nationalPrefixExtracted.length() > 0) {
			...
			// prefixBeforeNationalNumber. We cannot simply set
			// it to empty string because people sometimes enter national prefix
			// after country code, e.g
			// +44 (0)20-1234-5678.
			int indexOfPreviousNdd = prefixBeforeNationalNumber
					.lastIndexOf(nationalPrefixExtracted);
			prefixBeforeNationalNumber.setLength(indexOfPreviousNdd);
		}
		return !nationalPrefixExtracted
				.equals(removeNationalPrefixFromNationalNumber());
	}

	private boolean isDigitOrLeadingPlusSign(char nextChar) {
		return Character.isDigit(nextChar)
				|| (accruedInput.length() == 1 && PhoneNumberUtil.PLUS_CHARS_PATTERN
						.matcher(Character.toString(nextChar)).matches());
	}

	/**
	 * Check to see if there is an exact pattern match for these digits. If so,
	 * we should use this instead of any other formatting template whose
	 * leadingDigitsPattern also matches the input.
	 */
	String attemptToFormatAccruedDigits() {
		for (NumberFormat numberFormat : possibleFormats) {
			Matcher m = regexCache
					.getPatternForRegex(numberFormat.getPattern()).matcher(
							nationalNumber);
			if (m.matches()) {
				shouldAddSpaceAfterNationalPrefix = NATIONAL_PREFIX_SEPARATORS_PATTERN
						.matcher(numberFormat.getNationalPrefixFormattingRule())
	...
	/**
	 * Returns the current position in the partially formatted phone number of
	 * the character which was previously passed in as the parameter of
	...
	public int getRememberedPosition() {
		...
		while (accruedInputIndex < positionToRemember
			...
			if (accruedInputWithoutFormatting.charAt(accruedInputIndex) == currentOutput
	 ...
	 * national prefix) that was collected. A space will be inserted between
	 * them if the current formatting template indicates this to be suitable.
	 */
	private String appendNationalNumber(String nationalNumber) {
		int prefixBeforeNationalNumberLength = prefixBeforeNationalNumber
				.length();
		if (shouldAddSpaceAfterNationalPrefix
				&& prefixBeforeNationalNumberLength > 0
				&& prefixBeforeNationalNumber
						.charAt(prefixBeforeNationalNumberLength - 1) != SEPARATOR_BEFORE_NATIONAL_NUMBER) {
			// We want to add a space after the national prefix if the national
			// prefix formatting rule
			// indicates that this would normally be done, with the exception of
			// the case where we already
			// appended a space because the NDD was surprisingly long.
			return new String(prefixBeforeNationalNumber)
					+ SEPARATOR_BEFORE_NATIONAL_NUMBER + nationalNumber;
		} else {
	...
	private String attemptToChooseFormattingPattern() {
		// We start to attempt to format only when as least
		// MIN_LEADING_DIGITS_LENGTH digits of national
		// number (excluding national prefix) have been entered.
		if (nationalNumber.length() >= MIN_LEADING_DIGITS_LENGTH) {
			getAvailableFormats(nationalNumber.substring(0,
					MIN_LEADING_DIGITS_LENGTH));
			return maybeCreateNewTemplate() ? inputAccruedNationalNumber()
					: accruedInput.toString();
		} else {
	...
	/**
	 * Invokes inputDigitHelper on each digit of the national number accrued,
	...
	private String removeNationalPrefixFromNationalNumber() {
		...
		if (isNanpaNumberWithNationalPrefix()) {
			...
			Pattern nationalPrefixForParsing = regexCache
					.getPatternForRegex(currentMetaData
							.getNationalPrefixForParsing());
			Matcher m = nationalPrefixForParsing.matcher(nationalNumber);
			if (m.lookingAt()) {
				// When the national prefix is detected, we use international
				// formatting rules instead of
				// national ones, because national formatting rules could
				// contain local formatting rules
				// for numbers entered without area code.
				isCompleteNumber = true;
				startOfNationalNumber = m.end();
				prefixBeforeNationalNumber.append(nationalNumber.substring(0,
						startOfNationalNumber));
			}
		}
		String nationalPrefix = nationalNumber.substring(0,
				startOfNationalNumber);
	 ...
	 * 
	 * @return true when accruedInputWithoutFormatting begins with the plus sign
	 *         or valid IDD for defaultCountry.
	 */
	private boolean attemptToExtractIdd() {
		Pattern internationalPrefix = regexCache.getPatternForRegex("\\"
				+ PhoneNumberUtil.PLUS_SIGN + "|"
				+ currentMetaData.getInternationalPrefix());
		Matcher iddMatcher = internationalPrefix
				.matcher(accruedInputWithoutFormatting);
		if (iddMatcher.lookingAt()) {
			...
			prefixBeforeNationalNumber.setLength(0);
			prefixBeforeNationalNumber.append(accruedInputWithoutFormatting
	 ...
	 * prefixBeforeNationalNumber when they are available, and places the
	 * remaining input into nationalNumber.
	 * 
	...
	// Accrues digits and the plus sign to accruedInputWithoutFormatting for
	// later use. If nextChar
	// contains a digit in non-ASCII format (e.g. the full-width version of
	// digits), it is first
	// normalized to the ASCII version. The return value is nextChar itself, or
	// its normalized
	// version, if nextChar is a digit in non-ASCII format. This method assumes
	// its input is either a
	...
	private char normalizeAndAccrueDigitsAndPlusSign(char nextChar,
		...
		if (nextChar == PhoneNumberUtil.PLUS_SIGN) {
			...
			int radix = 10;
			normalizedChar = Character.forDigit(
					Character.digit(nextChar, radix), radix);
	...
	private String inputDigitHelper(char nextChar) {
		...
		if (digitMatcher.find(lastMatchPosition)) {
			String tempTemplate = digitMatcher.replaceFirst(Character
					.toString(nextChar));
